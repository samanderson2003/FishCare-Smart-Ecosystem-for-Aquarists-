...
const BreedingSuppliesScreen = ({ navigation }) => {
  const [supplies, setSupplies] = useState([]);
  const [cart, setCart] = useState([]);  // ✅ Local state for instant cart updates
  const [loading, setLoading] = useState(true);

  // ✅ Fetch Supplies in Real-time
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "BreedingSupplies"), (snapshot) => {
      const supplyList = snapshot.docs.map((doc) => {
        const data = doc.data();
        return {
          id: doc.id,
          name: data.name || "Unknown Supply",
  ...
  return (
    <LinearGradient colors={["#141E30", "#243B55"]} style={styles.background}>
      <View style={styles.container}>
        <Text style={styles.title}>Breeding Supplies</Text>
        <Text style={styles.subtitle}>Browse all available breeding supplies.</Text>
...
export default BreedingSuppliesScreen;