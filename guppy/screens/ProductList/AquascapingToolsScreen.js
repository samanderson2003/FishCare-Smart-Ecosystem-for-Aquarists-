...
const AquascapingToolsScreen = ({ navigation }) => {
  const [tools, setTools] = useState([]);
  const [cart, setCart] = useState([]);  // Local state for instant cart updates
  const [loading, setLoading] = useState(true);

  // Fetch Tools in Real-time
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "AquascapingTools"), (snapshot) => {
  ...
  return (
    <LinearGradient colors={["#141E30", "#243B55"]} style={styles.background}>
      <View style={styles.container}>
        <Text style={styles.title}>Aquascaping Tools</Text>
        <Text style={styles.subtitle}>Browse all available aquascaping tools.</Text>
...
export default AquascapingToolsScreen;