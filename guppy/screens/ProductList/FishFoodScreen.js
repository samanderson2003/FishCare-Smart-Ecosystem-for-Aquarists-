...
const FishFoodScreen = ({ navigation }) => {
  const [fishFoods, setFishFoods] = useState([]);
  const [cart, setCart] = useState([]);  // Local state for instant cart updates
  const [loading, setLoading] = useState(true);

  // Fetch Fish Food Products in Real-time
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "FishFood"), (snapshot) => {
      const fishFoodList = snapshot.docs.map((doc) => {
        const data = doc.data();
        return {
          id: doc.id,
          name: data.name || "Unknown Fish Food",
  ...
  return (
    <LinearGradient colors={["#141E30", "#243B55"]} style={styles.background}>
      <View style={styles.container}>
        ...
        <Text style={styles.subtitle}>Browse all available fish food products.</Text>
