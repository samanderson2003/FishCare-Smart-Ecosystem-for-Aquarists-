...
const AquariumPlantsScreen = ({ navigation }) => {
  const [plants, setPlants] = useState([]);
  const [cart, setCart] = useState([]);  // Local state for instant cart updates
  const [loading, setLoading] = useState(true);

  // Fetch Plants in Real-time
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "AquariumPlants"), (snapshot) => {
  ...
  return (
    <LinearGradient colors={["#141E30", "#243B55"]} style={styles.background}>
      <View style={styles.container}>
        <Text style={styles.title}>Aquarium Plants</Text>
        <Text style={styles.subtitle}>Browse all available aquarium plants.</Text>
...
export default AquariumPlantsScreen;