...
const TankAccessoriesScreen = ({ navigation }) => {
  const [accessories, setAccessories] = useState([]);
  const [cart, setCart] = useState([]);  // Local state for instant cart updates
  const [loading, setLoading] = useState(true);

  // Fetch Tank Accessories in Real-time
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "TankAccessories"), (snapshot) => {
      const accessoryList = snapshot.docs.map((doc) => {
        const data = doc.data();
        return {
          id: doc.id,
          name: data.name || "Unknown Accessory",
  ...
  return (
    <LinearGradient colors={["#141E30", "#243B55"]} style={styles.background}>
      <View style={styles.container}>
        <Text style={styles.title}>Tank & Accessories</Text>
        <Text style={styles.subtitle}>Browse all available tank accessories.</Text>
...
export default TankAccessoriesScreen;